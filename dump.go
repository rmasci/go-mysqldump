...
// Dump data using struct. fname is the name of the file to dump to.
func (data *Data) Dump(a ...interface{}) error {
	var tables []string
	meta := metaData{
		DumpVersion: Version,
	}
	...
	*/
	...
	// If nothing passed, just get a list of tables. Dump everything.
	if len(a) <= 0 {
		var err error
		...
		b := fmt.Sprintf("%v", a[0])
		// If the a ...Interface{} starts with a -, then we need to append to Data.IgnoreTables.
		if strings.HasPrefix(b, "-") {
			var err error
			//get rid of dash
			b = strings.TrimLeft(b, "-")
			ignore := strings.Split(b, ",")
			data.IgnoreTables = append(data.IgnoreTables, ignore...)
			tables, err = data.getTables()
			if err != nil {
				return err
			}
			...
			for _, x := range strings.Split(b, ",") {
				tables = append(tables, strings.TrimSpace(x))
			}
		}
	}
